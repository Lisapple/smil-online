...
function submit() {
	...
	var url = "http://smil.lisacintosh.com/parser.php?p=" + encodeURIComponent(p) + "&v=" + verbose;
	...
	url = "http://smil.lisacintosh.com/smileys.php?p=" + encodeURIComponent(p) + "&golf=" + golfed;
...
function smileysStateChange() {
	if (smileysRequest && smileysRequest.readyState == 4 &&
		...
		str += "<br>";
		str += "<a target=\"_blank\" href=\"http://smil.lisacintosh.com/export-smileys.php?p=" + encodeURIComponent(p) + "&columns=6\">Export to PNG</a>";
...
function loadProgram() {
	...
	document.getElementById("arguments").value = programs[name]["a"];
	document.getElementById("program").value = programs[name]["p"];
}